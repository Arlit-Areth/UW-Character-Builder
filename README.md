...
<div class="summary-panel">
  <div class="summary-top">
    ...
    <div class="summary-right">
      ...
      <div style="padding-top:6px">
        <button onclick="openRandomize()" style="width:100%;padding:7px 16px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-primary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:12px;font-weight:500;cursor:pointer">🎲 Randomize</button>
      </div>
      <div style="padding-top:6px">
...
<!-- Randomize Popup -->
<div id="randomize-overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:4500;align-items:center;justify-content:center;padding:1rem">
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-lg);padding:1.5rem;max-width:420px;width:100%">
    <div style="font-size:15px;font-weight:600;margin-bottom:0.4rem">🎲 Randomize Character</div>
    <div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:1.25rem;line-height:1.5">
      Set a target level and occupation. Blankets will be calculated automatically, then skills will be randomly assigned until CP is spent.
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:1rem">
      <div>
        <label style="font-size:12px;font-weight:500;display:block;margin-bottom:5px">Target Level</label>
        <input id="rand-level" type="number" min="1" max="100" value="10"
          style="width:100%;padding:8px 10px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px;box-sizing:border-box">
      </div>
      <div>
        <label style="font-size:12px;font-weight:500;display:block;margin-bottom:5px">Occupation</label>
        <select id="rand-occ"
          style="width:100%;padding:8px 10px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px;box-sizing:border-box">
    ...
    <div id="rand-status" style="font-size:12px;min-height:18px;margin-bottom:0.75rem;color:var(--color-text-secondary)"></div>
    <div style="display:flex;gap:8px;justify-content:flex-end">
      <button onclick="closeRandomize()" style="padding:8px 18px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px;cursor:pointer">Cancel</button>
      <button onclick="doRandomize()" style="padding:8px 18px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-primary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px;font-weight:500;cursor:pointer">🎲 Randomize</button>
    </div>
  </div>
</div>
...
<!-- Randomize Popup -->
<div id="randomize-overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:2600;align-items:center;justify-content:center;padding:1rem">
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-lg);padding:1.5rem;max-width:440px;width:100%">
    <div style="font-size:15px;font-weight:600;margin-bottom:0.25rem">🎲 Randomize Character</div>
    <div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:1.25rem;line-height:1.5">Sets occupation and blankets to reach the chosen level, then randomly spends all available CP.</div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:1rem">
      <div>
        <label style="font-size:12px;font-weight:500;display:block;margin-bottom:5px">Occupation</label>
        <select id="rand-occ" style="width:100%;padding:7px 10px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px">
          <option value="">— select —</option>
          <option>Mercenary</option><option>Ranger</option><option>Templar</option>
          <option>Nightblade</option><option>Assassin</option><option>Wytchhunter</option>
          <option>Mage</option><option>Druid</option><option>Bard</option>
      ...
      <div>
        <label style="font-size:12px;font-weight:500;display:block;margin-bottom:5px">Target Level</label>
        <input id="rand-level" type="number" min="1" max="50" value="10"
          style="width:100%;padding:7px 10px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans);font-size:13px;box-sizing:border-box">
      </div>
    </div>
...
const SKILLS = [
  ...
  {cat:'Rogue', skills:[
    skill('Critical +2: Specific',            [150,125,130,120,100,130,230,180,130],[], {_weaponChoice:'specific', desc:'This skill allows the player to swing for +2 damage when striking a target from behind and +½ damage from the front, with a single weapon that the character is proficient in, chosen at purchase. The attack must hit the target on the back of their body (Upper Back, Mid Back, or Lower Back locations) to grant the +2 damage. The damage call is "<damage> Critical" when striking from behind. This skill is always active.'}),
    skill('Critical +2: Group',               [170,145,150,140,120,150,250,200,150],[], {_weaponChoice:'group', desc:'This skill is the same as Critical +2: Specific, except that it is purchased for an entire Weapon Group rather than a specific weapon. The Critical may be used with any of the weapons within that Weapon Group.'}),
...
function printCharacterSummary(){
  ...
  document.getElementById('char-summary-body').innerHTML = html;
...
// ---- Randomize ----
function openRandomize(){
  document.getElementById('rand-status').textContent = '';
  if(s.occupation) document.getElementById('rand-occ').value = s.occupation;
  document.getElementById('randomize-overlay').style.display = 'flex';
}

function closeRandomize(){
  document.getElementById('randomize-overlay').style.display = 'none';
}

function doRandomize(){
  const occ = document.getElementById('rand-occ').value;
  const targetLevel = Math.max(1, parseInt(document.getElementById('rand-level').value)||1);
  const statusEl = document.getElementById('rand-status');

  if(!occ){
    statusEl.textContent = 'Please select an occupation.';
    return;
  }

  // Reset state, preserving name/race/culture
  s.owned = [];
  s.vocation = '';
  ...
  // Update DOM inputs
  const occEl = document.getElementById('sel-occ');
  if(occEl) occEl.value = occ;

  // Apply racial auto skills if race set
  if(s.race){
    getRacialAutoSkills().forEach(sk=>{
      if(!s.owned.find(o=>o.name===sk.name))
        s.owned.push({...sk, _auto:true, cp:0, frag:sk.frag||0});
    });
  }

  // Calculate blankets to reach target level
  // Find the CP threshold for targetLevel
  const rd = getRaceData();
  const bonus = rd ? (rd.cpBonus||0) : 0;
  const lastRow = TABLE[TABLE.length-1];
  const prevRow = TABLE[TABLE.length-2];
  const CPL = lastRow.threshold - prevRow.threshold; // 100 per level past 20
  let targetCP;
  if(targetLevel <= lastRow.level){
    const row = TABLE.find(r=>r.level===targetLevel);
    targetCP = (row ? row.threshold : 150) + bonus;
  } else {
    targetCP = lastRow.threshold + (targetLevel - lastRow.level) * CPL + bonus;
  }
  const baseCPWithBonus = 150 + bonus;
  const deficit = Math.max(0, targetCP - baseCPWithBonus);
  const blankets = deficit > 0 ? blanketsNeededToEarnCP(baseCPWithBonus, deficit) : 0;
  s.blankets = blankets;
  const blEl = document.getElementById('inp-blankets');
  if(blEl) blEl.value = blankets;
  ...
  const purchased = s.owned.filter(o=>!o._auto).length;
  statusEl.innerHTML = `<span style="color:var(--color-text-success,#7cb87c)">\u2713 Level ${getLevelFromCP(cpTotal).level} ${occ} \u2014 ${purchased} skills purchased, ${cpRem} CP remaining.</span>`;
...
async function submitBugReport(){
  ...
  document.getElementById('bug-status').innerHTML = '<span style="color:var(--color-text-success)">✓ GitHub opened — review and click "Submit new issue".</span>';
